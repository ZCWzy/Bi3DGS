...
def training(dataset, opt, pipe, args):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if progress > 0.5:
            # 映射进度：0.5 -> 0.0, 1.0 -> 1.0
            progress_bin = (progress - 0.5) / 0.5
            # 使用平方增长，使过渡更平滑 (从 0 逐渐增加到 1.0)
            w_bin = 0.5 * (progress_bin ** 2) 
        else:
            w_bin = 0.0
        ...
        # 只有当进度 > 0.75 时才开启
        if progress > 0.75:
            # 重新映射进度：从 0.75~1.0 映射到 0.0~1.0
            progress_iso = (progress - 0.75) / 0.25
            # 后期发力：从 0 逐渐增加到 0.1
            w_iso = 0.1 * (progress_iso)
        else:
            w_iso = 0.0
        ...
        # 透明度惩罚项 (后 50% 生效)
        # if w_bin > 0:
        #     opacity = torch.sigmoid(gaussians.get_opacity)
        #     # 增加 epsilon (1e-7) 防止 sqrt(0) 导致梯度爆炸 (NaN)
        #     binarize_loss = torch.sqrt(opacity * (1.0 - opacity) + 1e-7)
        #     loss += w_bin * binarize_loss.mean()

        # 各向同性损失 (后 25% 生效)
        if w_iso > 0:
            scaling = gaussians.get_scaling
            # 使用 keepdim=True 简化代码
            isotropic_loss = torch.abs(scaling - scaling.mean(dim=1, keepdim=True))
            loss += w_iso * isotropic_loss.mean()
        #####################################################
